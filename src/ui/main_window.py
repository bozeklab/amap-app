...
class MainWindow(QMainWindow):
    ...
    def configure_ui(self):
        ...
        self.button_results = self.findChild(QPushButton, "button_results")
    ...
    def activate_selected_project(self, _selected_item):
        ...
        self.button_results.setEnabled(project_configs['is_finished'])
        self.button_start.setEnabled(not project_configs['is_finished'])
    ...
    def start_project_click(self):
        ...
        self.progress_dialog = create_progress_dialog(f'Calculating...', 'Please wait', self)

        project_name = self.list_projects.currentItem().text()
        project_configs_path = f'./{PROJECT_DIR}/{project_name}/conf.json'
        project_configs = self.load_project_configuration(project_configs_path)
        self.engine = AMAPEngine(project_configs)
        self.project_process = mp.Process(target=self.start_project_procedure)
    ...
    def check_project_status(self):
        if self.project_process is None or self.project_process.is_alive():
            if self.engine is not None:
                percentage = self.engine.no_of_processed_tiles[0] / len(self.engine.dataset) * 100
                self.progress_dialog.setValue(percentage)
                if percentage > 95:
                    QtCore.QTimer.singleShot(1000, self.check_project_status)
                    return
            QtCore.QTimer.singleShot(500, self.check_project_status)
        else:
    ...
    def start_project_procedure(self):
        if self.engine:
            self.engine.exec()
    ...
    def add_project_click(self):
        ...
        project_configuration = {
            ...
            "result_dir": f"./{PROJECT_DIR}/{selected_path.name}/output/",
    ...
    def restore_UI_state(self, _UI_state):
        ...
        self.button_results.setEnabled(_UI_state[7])
